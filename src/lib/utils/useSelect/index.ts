...
>(
	...
	const toValue = (v: unknown) => {
		return (isObject(v) ? (v as T)[itemValue as keyof T] : v) as Value<T, K, R>
	}
	...
	const isSelected = (item: Value<T, K, R>, $selected = getStoreValue(selected)) => {
		if ([true, undefined].includes(returnObject)) {
			return multiple ? ($selected as T[]).includes(item as T) : item === $selected
		}

		const value = toValue(item)
		return multiple ? ($selected as Value<T, K, R>[]).includes(value) : value === $selected
	}
